...
import {
  Search, SlidersHorizontal, Clock, Video, Users, MapPin,
  ChevronLeft, ChevronRight, CalendarDays, Plus
} from 'lucide-react';
import { useGetVendorMeetingsQuery } from '../services/vendorApi';

// ─── Fallback mock data (shown when API hasn't connected yet) ─────────────────
const MOCK_AVATARS = [
...
const MOCK_MEETINGS = [
  {
    id: 1, type: 'Vendor Review', label: 'Upcoming Meeting',
    title: 'Vendor Onboarding Review',
    date: '28 March', time: '10:00 am', duration: '45 mins', mode: 'Zoom', participants: 4,
    avatars: MOCK_AVATARS.slice(0, 2),
  },
  {
    id: 2, type: 'Contract Meeting', label: 'Upcoming Meeting',
    title: 'Contract Renewal Discussion',
    date: '30 March', time: '2:00 pm', duration: '60 mins', mode: 'Office', participants: 6,
    avatars: MOCK_AVATARS.slice(1, 4),
  },
  {
    id: 3, type: 'Vendor Review', label: 'Upcoming Meeting',
    title: 'Quarterly Performance Sync',
    date: '2 April', time: '11:00 am', duration: '30 mins', mode: 'Zoom', participants: 5,
    avatars: MOCK_AVATARS.slice(0, 3),
  },
  {
    id: 4, type: 'Contract Meeting', label: 'Upcoming Meeting',
    title: 'SLA Compliance Review',
    date: '4 April', time: '3:00 pm', duration: '60 mins', mode: 'Zoom', participants: 7,
    avatars: MOCK_AVATARS.slice(2, 4),
  },
];
...
// ─── Meeting Card (shared UI component pattern) ───────────────────────────────
const MeetingCard = ({ meeting }) => {
  const typeColors = {
    'Vendor Review': 'bg-blue-50 text-blue-700',
    'Contract Meeting': 'bg-amber-50 text-amber-700',
  };
...
// ─── Main Vendor Meetings Page ─────────────────────────────────────────────────
const VendorMeetings = () => {
  // Vendor-specific API call — completely separate from client meetings
  const { data, isLoading, isError } = useGetVendorMeetingsQuery();

  // Use API data when available; fall back to mock data for development
  const meetings = data?.items ?? MOCK_MEETINGS;
    ...
    <div className="max-w-[1400px] mx-auto pb-12">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-xs text-slate-400">Manage and schedule vendor engagement meetings</p>
        </div>
      ...
      {isLoading && (
        <div className="py-8 text-center text-slate-400 text-sm">Loading meetings...</div>
      )}
      {isError && (
        <div className="py-4 mb-4 text-center text-amber-600 bg-amber-50 rounded-xl border border-amber-100 text-xs">
          Could not load meetings from API — showing mock data.
        </div>
      )}
        ...
        <div className="bg-white border border-slate-100 card-shadow rounded-2xl p-8 mb-10">
          <div className="flex gap-12">
            <div className="flex-1">
              ...
              <h2 className="text-2xl font-bold text-slate-800 mb-3">{nextMeeting.title}</h2>
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-400 mb-6">
                ...
                <span>•</span>
                <span>{nextMeeting.mode}</span>
                <span>•</span>
                <span>{nextMeeting.participants} Participants</span>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col items-end justify-between min-w-[200px]">
              ...
              <div className="bg-primary text-white text-center rounded-xl px-8 py-4 mt-6">
                ...
                <p className="text-xl font-bold font-serif tracking-wider">--:--:--</p>
              </div>
            </div>
          </div>
        </div>
